...
#include "kalloc.h"
#include "dawg.h"
#include "io.h" // for rb3_sprintf_lite()
#define kh_packed
#include "khashl-km.h"
...
static void sw_cs_core(rb3_swhit_t *hit, const uint8_t *qseq, int len_only) // call after sw_backtrack1()
{
	kstring_t *s = 0, out = {0,0,0};
	int32_t i, k, cs_len = 0, x = 0, y = hit->qoff[0];
	if (!len_only) {
		assert(hit->cs_len > 0);
		out.m = hit->cs_len + 1, out.s = hit->cs;
		s = &out;
	}
	for (k = 0; k < hit->n_cigar; ++k) {
		int32_t op = hit->cigar[k]&0xf, len = hit->cigar[k]>>4;
		if (op == 7) {
			cs_len += rb3_sprintf_lite(s, ":%d", len);
			x += len, y += len;
		} else if (op == 8) {
			for (i = 0; i < len; ++i)
				cs_len += rb3_sprintf_lite(s, "*%c%c", "$acgtn"[qseq[y+i]], "$acgtn"[hit->rseq[x+i]]);
			x += len, y += len;
		} else if (op == 1) {
			cs_len += rb3_sprintf_lite(s, "+");
			for (i = 0; i < len; ++i)
				cs_len += rb3_sprintf_lite(s, "%c", "$acgtn"[qseq[y+i]]);
			y += len;
		} else if (op == 2) {
			cs_len += rb3_sprintf_lite(s, "-");
			for (i = 0; i < len; ++i)
				cs_len += rb3_sprintf_lite(s, "%c", "$acgtn"[hit->rseq[x+i]]);
			x += len;
		} else assert(0);
	}
	hit->cs_len = cs_len;
...
void rb3_sw(void *km, rb3_sai_v *mem, const rb3_swopt_t *opt, const rb3_fmi_t *f, int len, const uint8_t *seq, rb3_swrst_t *rst)
{
	...
	if (f->ssa) {
		...
		for (k = 0; k < rst->n; ++k) {
			...
			hit->n_pos = rb3_ssa_multi(km, f, f->ssa, hit->lo, hit->hi, n, hit->pos, opt->flag & RB3_SWF_SPEC);
            rest -= hit->n_pos;
            if (opt->flag & RB3_SWF_MAX_HIS) {  // get reference name/id and position for these hits in the SA interval, added by Schaudge King
                hit->rhs = RB3_CALLOC(char, n * 16 + 1);  // max accession size + one comma = 16
                hit->rhc = RB3_CALLOC(uint32_t, n + 1);
                kstring_t out = {0,0,0};
                out.m = n * 16 + 1, out.s = hit->rhs;
                int64_t idx = 0, space_used = 0;
                if (hit->lo + n < hit->hi) {  // set the first taxonomy id, hit count to 0, opt->max_hc + 1 respectively for hits beyond max_hc
                ...
                for (; idx < n; ++idx) {
                    ...
                    uint32_t find = 0, sid_len = strlen(f->sid->name[hit->pos[idx].sid>>1]);
                    uint32_t ii = 0, jj = 1, ci = 0;
                    for (; find < 1 && jj < space_used; ++jj)
                        if (*(hit->rhs + jj) == ',') {
                            if (ii + sid_len == jj && strncmp(f->sid->name[hit->pos[idx].sid>>1], hit->rhs + ii, sid_len) == 0) {
                                hit->rhc[ci] += 1;
                                find = 1;
                            }
                            ++ci;
                    ...
                    if (!find) {  // find == 0
                        space_used += rb3_sprintf_lite(&out, "%s,", f->sid->name[hit->pos[idx].sid>>1]);
                        hit->rhc[ci] = 1;
                    }
                }
            }
